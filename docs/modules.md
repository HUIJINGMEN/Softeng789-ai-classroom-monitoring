...
| Attendance & sessions | `AttendanceController`, `ClassroomSessionController`, `AttendanceService`, `ClassroomSessionService`; entities `ClassroomSession`, `AttendanceRecord`; bounded batch reads avoid one HTTP request per historical session | `Attendance.tsx`, `SessionDetail.tsx`, `useSessionAttendance.ts`, `features/session-attendance/`, `classroomApi.ts`, `attendanceAnalytics.ts` |
